...
(defun calculate-location (a b kimberly bob jack janet)
  (let* ((1/a^2 (eval-formula (1 / (expt a 2))))
         (1/b^2 (eval-formula (1 / (expt b 2))))

         (Pax   (eval-formula ((1/a^2 + 1) + (1/b^2 + 1))))
         (Pbx   (eval-formula ((2 * (location-x bob)   - 2 * (location-x kimberly) * 1/a^2) +
                               (2 * (location-x janet) - 2 * (location-x jack) * 1/b^2))))
         (Pcx   (eval-formula (((expt (location-x kimberly) 2) * 1/a^2 - (expt (location-x bob) 2)) +
                               ((expt (location-x jack) 2) * 1/b^2 - (expt (location-x janet) 2)))))

         (Pay   (eval-formula ((1/a^2 - 1) + (1/b^2 - 1))))
         (Pby   (eval-formula ((2 * (location-y bob) - 2 * (location-y kimberly) * 1/a^2) +
                               (2 * (location-y janet) - 2 * (location-y jack) * 1/b^2))))
         (Pcy   (eval-formula (((expt (location-y kimberly) 2) * 1/a^2 - (expt (location-y bob) 2)) +
                               ((expt (location-y jack) 2) * 1/b^2 - (expt (location-y janet) 2)))))

         (Kx    (eval-formula (((expt Pbx 2) / (4 * Pax)) - Pcx)))
         (Ky    (eval-formula (((expt Pby 2) / (4 * Pay)) - Pcy))))
